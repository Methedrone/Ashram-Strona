...
**Wersja:** 5 (finalna, zrealizowana 2026-08-10) · **Status: ✅ WYKONANE** (release dev → master: PR #48, deploy 2026-08-10, weryfikacja live 22/22)
...
| F6 | Weryfikacja live (CF Pages: manifest/favicon/avif 200 + MD5, 22/22 stron identycznych), audyt spójności (RAPORT-audyt-dev.md), release PR dev→master #48 + deploy | #48 | ✅ |
...
- F6 live: produkcja po deploy == lokalny build **22/22** (normalizacja: GTM first-party /lhsi, CF email obfuscation, hashe astro-lightgallery); manifest/favicon/avif **200 + MD5 identyczny**
- GTM-5J4NL66W renderuje się na produkcji z first-party URL (`/lhsi/gtm.js` — hotfix #41 zachowany przez merge)
...
- **dev nie zawiera hotfixu #41 (GTG first-party /lhsi)** — master go ma; cherry-pick #41 na dev dla spójności (inaczej przyszłe zmiany Layout na dev mogą wywołać konflikt i cofnąć first-party)
- `/lhsi` 404 na ścieżce: Redirect Rule w CF lub wyłączenie first-party mode w GTM (patrz RAPORT-audyt-dev.md) — GTM już ładuje gtm.js z /lhsi, ale ścieżka nie jest routowana
...
- CF Email Obfuscation owija gołe emaile w linki mailto (np. /donations) — świadome zachowanie CF, do decyzji (można wyłączyć per-strona)
